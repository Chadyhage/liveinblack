...
import { regionToCurrency, payRailLabel } from '../utils/money'
  ...
  return <Layout><div className="org-studio">
    ...
    <div className="os-layout">
      <section className="os-panel"><h2>Informations publiques</h2>
        ...
        <div className="os-grid"><Field label="Nom public"><input style={input} value={profile.publicName||''} onChange={e=>update({publicName:e.target.value})}/></Field><Field label="Slug public"><input style={{...input,borderColor:slugCheck.ok?'rgba(255,255,255,.12)':'rgba(224,90,170,.6)'}} value={profile.slug||''} onChange={e=>update({slug:e.target.value})}/>{!slugCheck.ok&&<small style={{color:C.pink}}>{slugCheck.error}</small>}</Field><Field label="Ville d’intervention"><input style={input} placeholder="Ta ville de base" value={profile.city||''} onChange={e=>update({city:e.target.value})}/></Field><Field label="Pays / régions d’intervention" wide><div style={{display:'flex',flexWrap:'wrap',gap:8}}>{ZONE_OPTIONS.map(r=>{const sel=zones.includes(r.id);return <button key={r.id} type="button" onClick={()=>toggleZone(r.id)} style={{padding:'8px 14px',borderRadius:20,border:`1px solid ${sel?C.teal:'rgba(255,255,255,.14)'}`,background:sel?'rgba(78,232,200,.14)':'rgba(255,255,255,.06)',color:sel?C.teal:'rgba(255,255,255,.6)',fontFamily:FONT,fontSize:12.5,fontWeight:600,cursor:'pointer',letterSpacing:'.02em'}}>{r.flag} {r.name}</button>})}</div><small style={{color:'rgba(255,255,255,.42)',fontSize:11,marginTop:4}}>Sélectionne tous les pays où tu organises — les visiteurs pourront te trouver en cherchant l’un d’eux. C’est du marketing : ça ne change JAMAIS ta devise ni ton mode de paiement.</small></Field>
        <Field label="Zone de facturation" wide><div style={{display:'flex',alignItems:'center',gap:10,padding:'11px 14px',borderRadius:12,border:'1px solid rgba(200,169,110,.28)',background:'rgba(200,169,110,.06)'}}><span style={{fontSize:18}}>{regionToCurrency(profile.regionId||profile.country)==='XOF'?'📱':'💳'}</span><div style={{minWidth:0}}><p style={{font:'700 12.5px Inter, sans-serif',color:C.gold,margin:0}}>{getRegionName(profile.regionId)||profile.country||'—'} · {regionToCurrency(profile.regionId||profile.country)==='XOF'?'FCFA (XOF)':'Euro (€)'}</p><p style={{font:'500 11px Inter, sans-serif',color:'rgba(255,255,255,.5)',margin:'2px 0 0',lineHeight:1.5}}>{payRailLabel(regionToCurrency(profile.regionId||profile.country))} — fixée à ton inscription. Tes prix et versements sont TOUJOURS dans cette devise, indépendamment des pays d’intervention ci-dessus.</p></div></div></Field><Field label="Description" wide><textarea style={input} rows={4} maxLength={500} placeholder="Présente ton univers en quelques phrases." value={profile.shortDescription||profile.longDescription||''} onChange={e=>update({shortDescription:e.target.value,longDescription:''})}/></Field><Field label="Réseaux sociaux" wide><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>{SOCIAL_NETWORKS.map(net=><label key={net.key} style={{display:'grid',gap:5}}><span style={{font:'600 10.5px Inter, sans-serif',letterSpacing:'.04em',textTransform:'uppercase',color:'rgba(255,255,255,.5)'}}>{net.label}</span><input style={input} placeholder={net.placeholder} value={profile.socialLinks?.[net.key]||''} onChange={e=>update({socialLinks:{...(profile.socialLinks||{}),[net.key]:e.target.value}})}/></label>)}</div><small style={{color:'rgba(255,255,255,.42)',fontSize:11,marginTop:6,display:'block'}}>Colle un lien complet ou juste ton @pseudo — le lien fonctionnera au clic pour tes visiteurs. Laisse vide ce que tu n’utilises pas.</small></Field></div>
