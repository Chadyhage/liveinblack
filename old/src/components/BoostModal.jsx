...
function RankIcon({ position, size = 20 }) {
  ...
  return <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
    ...
    <text x="12" y="15" textAnchor="middle" fontSize="8" fill="#090a10" fontFamily="Inter, sans-serif" fontWeight="bold">{position}</text>
  </svg>
}
...
export default function BoostModal({ event, onClose, onBoostDone }) {
  ...
  return <div className="boost-shell" role="dialog" aria-modal="true" aria-label="Booster mon événement">
    <style>{`
      ...
      .boost-backdrop{position:absolute;inset:0;background:rgba(3,4,8,.72);backdrop-filter:blur(8px)}
      .boost-panel{position:relative;width:100%;max-width:660px;max-height:calc(100vh - 36px);overflow:auto;background:#12131c;border:1px solid rgba(255,255,255,.10);border-radius:20px;box-shadow:0 24px 64px rgba(0,0,0,.55)}
      ...
      .boost-title{font:700 25px Inter,sans-serif;color:#fff;margin:0;letter-spacing:-.02em}
      .boost-kicker{font:600 11px ${mono};letter-spacing:.06em;text-transform:uppercase;color:#c8a96e;margin:7px 0 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:430px}
      ...
      .boost-label{font:700 11px ${mono};letter-spacing:.06em;text-transform:uppercase;color:rgba(255,255,255,.55);margin:0 0 10px}
      ...
      .boost-position-top{display:flex;align-items:center;gap:8px;font:600 13px ${mono}}
      .boost-position-state{display:block;font:700 11px ${mono};letter-spacing:.05em;text-transform:uppercase;margin-top:9px}
      .boost-duration-head{display:flex;justify-content:space-between;align-items:end;gap:12px;margin-bottom:10px}
      .boost-description{font:12px ${mono};color:rgba(255,255,255,.5);margin:0}
      ...
      .boost-duration span{display:block;font:600 11px ${mono};letter-spacing:.05em;text-transform:uppercase}
      .boost-duration strong{display:block;font:700 22px Inter,sans-serif;margin-top:10px;white-space:nowrap}
      .boost-primary{min-height:52px;width:100%;border-radius:12px;border:none;background:#c8a96e;color:#181206;font:700 14px ${mono};cursor:pointer;box-shadow:0 6px 20px rgba(200,169,110,.25)}
      .boost-primary:disabled{background:rgba(255,255,255,.07);color:rgba(255,255,255,.35);box-shadow:none;cursor:not-allowed}
      .boost-summary{padding:18px;border:1px solid rgba(200,169,110,.3);border-radius:14px;background:rgba(255,255,255,.04)}
      .boost-row{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:8px 0;font:13px ${mono};color:rgba(255,255,255,.55)}
      .boost-row strong{color:#fff;font-weight:600}.boost-total{border-top:1px solid rgba(255,255,255,.08);margin-top:5px;padding-top:14px}.boost-total strong{font:700 25px Inter;color:#c8a96e}
      .boost-pay-actions{display:grid;grid-template-columns:1fr 2fr;gap:9px}.boost-ghost{min-height:50px;border-radius:12px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:rgba(255,255,255,.9);font:600 13px ${mono};cursor:pointer}
      @media(max-width:620px){.boost-shell{align-items:flex-end;padding:0}.boost-panel{max-height:92vh;border-radius:18px 18px 0 0}.boost-content{padding:20px 18px 28px}.boost-title{font-size:22px}.boost-duration-grid{grid-template-columns:repeat(2,1fr)}.boost-position{min-height:70px;padding:10px 8px}.boost-position-state{font-size:10px}.boost-kicker{max-width:260px}}
    `}</style>
    ...
    <section className="boost-panel">
      <div className="boost-content">
        ...
        {step === 'error' ? <div style={{textAlign:'center',padding:'28px 0'}}>
          <p style={{font:`13px ${mono}`,color:'#ee9bb7',lineHeight:1.7}}>{errorMsg}</p>
          ...
          <p style={{font:`12px ${mono}`,color:'rgba(255,255,255,.5)',lineHeight:1.7,margin:0}}>Paiement sécurisé via Stripe. Le créneau est confirmé uniquement après validation du paiement.</p>
          <div className="boost-pay-actions"><button className="boost-ghost" onClick={() => setStep('pick')}>Retour</button><button className="boost-primary" onClick={confirmBoost} disabled={paying}>{paying ? <span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',gap:8}}><span className="lib-spin" style={{width:14,height:14,border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'#fff',borderRadius:'50%',display:'inline-block'}}/>Redirection vers Stripe…</span> : `Payer ${chosenTier.price.toFixed(2).replace('.', ',')} €`}</button></div>
        </> : <>
          <div><p className="boost-label">1. Choisis ta position</p><div className="boost-position-grid">
            {BOOST_PLANS.map(plan => {
              ...
              return <button
                ...
                style={{
                  border: active ? '1px solid rgba(200,169,110,.65)' : '1px solid rgba(255,255,255,.08)',
                  background: active ? 'rgba(200,169,110,.14)' : 'rgba(255,255,255,.04)',
          ...
          <div><div className="boost-duration-head"><div><p className="boost-label" style={{marginBottom:5}}>2. Choisis la durée</p><p className="boost-description">{activePlan.description}</p></div>{positionBlocked(activePosition)&&<span style={{font:`700 11px ${mono}`,color:positionColor(activePosition),textTransform:'uppercase',letterSpacing:'0.04em'}}>{positionLabel(activePosition)}</span>}</div>
            <div className="boost-duration-grid">{activePlan.tiers.map((tier,index) => {
              ...
              return <button
                ...
                style={{
                  border: selected ? '1px solid rgba(78,232,200,.7)' : '1px solid rgba(255,255,255,.08)',
                  background: selected ? 'rgba(78,232,200,.12)' : 'rgba(255,255,255,.04)',
              ...
              >
                ...
                {exceeds&&<small style={{display:'block',font:`11px ${mono}`,color:'rgba(255,255,255,.5)',marginTop:5}}>Après l’événement</small>}
              </button>
            })}</div>
            {positionBlocked(activePosition)&&<p style={{font:`12px ${mono}`,lineHeight:1.6,color:'rgba(255,255,255,.5)',margin:'10px 0 0'}}>Cette place est déjà prise ou en cours de paiement dans cette région. Choisis une autre position.</p>}
          </div>
