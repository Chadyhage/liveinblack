...
import { IconCrown, IconCheck } from './icons'
    ...
    <div className="space-y-5">
      ...
      <div
        className={`glass p-4 rounded-2xl text-center transition-all ${
          bidFlash ? 'bg-[#c8a96e]/10 border-[#c8a96e]/30' : ''
        }`}
      >
        <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">Meilleure offre</p>
        <p className="text-3xl font-black text-[#c8a96e]" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>
          {fmtMoney(currentBid, aCur)}
        </p>
        <div className="flex items-center justify-center gap-2 mt-1">
          <span style={{ display: 'inline-flex' }}>
            <IconCrown size={14} color="#c8a96e" />
          </span>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl bg-[#c8a96e] text-[#181206] text-sm font-bold transition-all disabled:bg-white/10 disabled:text-white/35 disabled:cursor-not-allowed"
            >
              Enchérir
            </button>
          </div>
      ...
      {/* Group sent flash */}
      {groupSentFlash && (
        <div className="p-3 rounded-xl bg-[#0c0c16] border border-[#4ee8c8]/50 text-[#4ee8c8] text-sm font-semibold animate-fade-in flex items-center justify-center gap-2">
          <IconCheck size={14} color="#4ee8c8" />
      ...
      {showBidModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative w-full max-w-md bg-[#12131c] border border-white/10 rounded-t-3xl p-5 space-y-4 pb-8 shadow-[0_24px_64px_rgba(0,0,0,0.55)]">
            ...
            {!showGroupPicker ? (
              <>
                <div className="text-center space-y-1">
                  ...
                  <p className="text-[#c8a96e] text-3xl font-black" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>{fmtMoney(myBid, aCur)}</p>
                ...
                <div className="space-y-2">
                  <button
                    onClick={confirmBid}
                    className="w-full py-3.5 rounded-xl bg-[#c8a96e] text-[#181206] font-bold text-sm active:scale-95 transition-all"
                  >
                    Enchérir en solo — {fmtMoney(myBid, aCur)}
                  </button>
                  <button
                    onClick={() => { setShowGroupPicker(true); setGroupConvId(null) }}
                    className="w-full py-3.5 rounded-xl bg-white/10 border border-white/15 text-white/90 font-semibold text-sm active:scale-95 transition-all"
                  >
                    Proposer au groupe
                  </button>
                  <button onClick={() => { setShowBidModal(false); setWalletError(false) }} className="w-full py-2 text-gray-400 text-xs hover:text-white">
                    Annuler
                  </button>
                </div>
              </>
              ...
              <>
                <div>
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">Enchère de <span className="text-[#c8a96e] font-bold">{fmtMoney(myBid, aCur)}</span> — valable tant que le prix n'a pas dépassé ce montant</p>
                </div>
                <div className="space-y-2 max-h-52 overflow-y-auto">
                  {getConversations(userId).filter(c => c.type === 'group').length === 0 ? (
                    <p className="text-gray-500 text-xs text-center py-4">Aucun groupe dans ta messagerie pour l'instant.</p>
                    ...
                    >
                      <div className="w-9 h-9 rounded-full bg-[#1a1a1a] border border-[#333] flex items-center justify-center text-xs font-bold text-[#c8a96e] flex-shrink-0">
                        {getInitials(c.name || '?')}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm font-semibold truncate">{c.name}</p>
                        <p className="text-gray-500 text-[11px]">{c.members?.length || 0} membres</p>
                      </div>
                      {groupConvId === c.id && <IconCheck size={14} color="#60a5fa" />}
                    </button>
                  ))}
                </div>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${groupConvId ? 'bg-blue-600 text-white active:scale-95' : 'bg-white/10 text-white/35 cursor-not-allowed'}`}
                  >
                    Envoyer
                  </button>
                </div>
      ...
      {/* Bid history */}
      <div>
        <h4 className="text-gray-500 text-xs uppercase tracking-wider mb-3">Historique des enchères</h4>
        <div className="space-y-2">
          {bids.map((bid, i) => (
            <div
              key={i}
              className={`flex items-center justify-between p-3 rounded-xl border ${
                i === 0 ? 'border-[#c8a96e]/30 bg-[#c8a96e]/10' : 'border-white/[0.08] bg-[#0e0f16]'
              }`}
            >
              <div className="flex items-center gap-2">
                {i === 0 && (
                  <span style={{ display: 'inline-flex' }}>
                    <IconCrown size={12} color="#c8a96e" />
                  </span>
                )}
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-black"
                  style={{ background: i === 0 ? '#c8a96e' : '#333', color: i === 0 ? '#000' : '#888' }}
                >
                  {bid.user[0]}
                </div>
              ...
              <div className="text-right">
                <p className={`font-bold ${i === 0 ? 'text-[#c8a96e]' : 'text-gray-400'}`}>
      ...
      {isEnded && (
        <div className="p-4 rounded-2xl bg-[#0e0f16] border border-[#c8a96e]/30 text-center">
          <p className="text-[#c8a96e] font-bold flex items-center justify-center gap-2"><IconCrown size={16} color="#c8a96e" /> Enchère terminée</p>
