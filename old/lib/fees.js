...
export const SUBSCRIPTION = {
  ...
  // Zone FCFA (FedaPay) : le mobile money n'autorise PAS le prélèvement récurrent
  // automatique → modèle RENOUVELLEMENT MANUEL tous les 30 jours (paiement
  // ponctuel FedaPay). Voir lib/providerSubscription.js pour la logique de cycle.
  PRESTATAIRE_XOF: {
    amount: 9000,              // 9 000 FCFA / 30 jours (entier, zéro décimale)
    currency: 'xof',
    interval: 'month',
    implemented: true,
...
// Zone UEMOA (Togo, Bénin, Côte d'Ivoire, Sénégal, Burkina, Mali, Niger,
// Guinée-Bissau) → FCFA/XOF (FedaPay) ; France (et défaut) → EUR (Stripe).
// Miroir de src/utils/money.js — garder les deux alignés.
