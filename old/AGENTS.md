...
Tu travailles sur LIVEINBLACK, une marketplace événementielle nightlife.

Le dossier sélectionné contient tout le code source du projet.

CONTEXTE : Ce projet a été entièrement construit par Chady Hage (fondateur, non développeur) en collaboration avec Claude. Chady a décidé de déléguer le développement à un développeur web professionnel. Tu dois assister ce développeur exactement comme tu assistais Chady.

STACK : React 19 + Vite + Tailwind CSS + Firebase (Auth + Firestore + Storage) + Stripe + Vercel

ARCHITECTURE CLÉE : L'app utilise un write-through cache — localStorage (préfixe lib_) pour les lectures instantanées, Firestore pour la persistance cross-device. Comprendre firestore-sync.js est indispensable.

FICHIERS PRIORITAIRES :
- src/context/AuthContext.js
- src/utils/firestore-sync.js
- src/components/Layout.jsx
- src/utils/messaging.js

À NE PAS CASSER :
- handleProtectedNav() dans Layout.jsx
- voteOnPoll() gère 'poll' ET 'event_poll' ensemble
- Bottom nav mobile = position fixed (pas sticky)
- Imports firestore-sync = toujours dynamiques

PROBLÈMES EN COURS : sync Firestore cross-device non testée en prod, webhooks Stripe manquants, super admin hard-codé dans LoginPage.jsx.
