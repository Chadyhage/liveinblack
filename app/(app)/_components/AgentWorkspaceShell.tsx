...
export default function AgentWorkspaceShell({ children, badges }: { children: React.ReactNode; badges: Partial<Record<string, number>> }) {
  ...
  return <div className={styles.root}>
    <aside className={styles.sidebar}><Link href="/agent" className={styles.brand}><span className={styles.mark}>LB</span><span className={styles.brandText}><strong>LIVEINBLACK</strong><span>Console d’opérations</span></span></Link><nav className={styles.nav} aria-label="Administration">{navigation}</nav><div className={styles.sidebarFoot}>{session?.user ? <div className={styles.sidebarAccount}><AccountMenu user={session.user} dashboardMode /></div> : null}<Link href="/home" className={styles.publicLink}><Globe size={17}/><span>Voir le site public</span></Link></div></aside>
