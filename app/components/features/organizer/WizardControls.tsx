...
export function InputField({ label, value, onChange, placeholder, type = 'text', error, style, min, max, maxLength, locked = false }: {
  ...
  return (
    <div>
      ...
      <Input type={type} min={min} max={max} maxLength={maxLength} disabled={locked} invalid={Boolean(error)} title={locked ? 'Verrouillé — billets déjà vendus' : undefined} style={{ ...inputStyle, opacity: locked ? 0.55 : 1, background: locked ? 'rgba(255,229,0,0.04)' : inputStyle.background, ...style }} placeholder={placeholder} value={value} onChange={onChange} />
