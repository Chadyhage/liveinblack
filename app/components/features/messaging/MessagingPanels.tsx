  ...
  return (
    <ModalShell title="Contact" onClose={onClose} wide>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: online ? '#c8a96e' : 'var(--text-faint)', margin: 0 }}>{online ? 'En ligne' : lastSeenAt ? `Vu ${new Date(lastSeenAt).toLocaleString('fr-FR')}` : 'Hors ligne'}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {isBlocked ? <Button variant="secondary" onClick={onUnblock} style={{ ...fullRowButtonStyle, borderRadius: 10 }}>Débloquer</Button> : <Button variant="danger" onClick={onBlock} style={{ ...fullRowButtonStyle, borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border-strong)', color: '#ff7b7b' }}>Bloquer</Button>}
        <Button variant="danger" onClick={onReport} style={{ ...fullRowButtonStyle, borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border-strong)', color: '#ff7b7b' }}>Signaler</Button>
      </div>
    </ModalShell>
  )
}
