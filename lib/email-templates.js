...
// Email de vérification d'adresse (renvoyé par un admin depuis le panneau).
// `verifyLink` = lien généré par l'Admin SDK (getAuth().generateEmailVerificationLink).
export function emailVerificationEmail(verifyLink, site = DEFAULT_SITE) {
  const inner = `
...
// Email « nouvel événement » envoyé aux abonnés d'un organisateur (opt-in au
// moment de l'abonnement — cf. consentement affiché près du bouton S'abonner).
// Chaque destinataire reçoit SON e-mail (jamais de CC groupé). Le pied d'email
// rappelle pourquoi on écrit + où se désabonner (obligation d'opt-out claire).
export function newEventEmail(event, organizerName, site = DEFAULT_SITE) {
  const name = escapeHtml(organizerName || 'Un organisateur que tu suis')
  const evName = escapeHtml(event.name || 'Nouvel événement')
  const when = [event.dateDisplay || event.date, event.time].filter(Boolean).map(escapeHtml).join(' · ')
  const where = [event.venue, event.city].filter(Boolean).map(escapeHtml).join(', ')
  const inner = `
    ${h(evName, '#c8a96e')}
    ...
    ${event.description ? p(escapeHtml(String(event.description).slice(0, 220)) + (String(event.description).length > 220 ? '…' : '')) : ''}
    ${btn(`${site}/evenements/${encodeURIComponent(event.id)}`, 'Voir l\'événement et réserver')}
    ${p(`<span style="color:rgba(255,255,255,0.4);font-size:12px;">Tu reçois cet email parce que tu es abonné à ${name} sur LIVEINBLACK. Tu peux te désabonner ou régler tes alertes à tout moment depuis <a href="${site}/profil/organisateurs-suivis" style="color:rgba(78,232,200,0.7);">tes organisateurs suivis</a>.</span>`)}
  `
  return { subject: `${organizerName || 'LIVEINBLACK'} annonce : ${event.name || 'nouvel événement'}`, html: wrap(inner, site) }
}

// E-mail acheteur — ÉVÉNEMENT ANNULÉ. `refundNote` adapte le message au moyen de
// paiement (carte = auto ; mobile money = en cours de traitement).
export function eventCancelledEmail(event, { organizerName, refundNote } = {}, site = DEFAULT_SITE) {
  const evName = escapeHtml(event.name || 'ton événement')
  const when = [event.dateDisplay || event.date, event.time].filter(Boolean).map(escapeHtml).join(' · ')
  const orga = escapeHtml(organizerName || "L'organisateur")
  const inner = `
    ${h('Événement annulé', '#e05aaa')}
    ${p(`${orga} a dû <strong style="color:#ffffff;">annuler</strong> « ${evName} »${when ? ` (${when})` : ''}.`)}
    ${p('Ton billet n\'est plus valide et ne permettra pas l\'entrée.')}
    ${p(`<strong style="color:rgba(255,255,255,0.85);">Remboursement :</strong> ${escapeHtml(refundNote || 'Tu es intégralement remboursé du montant payé.')}`)}
    ${p('Toutes nos excuses pour ce contretemps — on espère te revoir très vite sur LIVEINBLACK.')}
    ${btn(`${site}/evenements`, 'Découvrir d\'autres événements', '#4ee8c8')}
  `
  return { subject: `Annulé : ${event.name || 'ton événement'}`, html: wrap(inner, site) }
}

// E-mail acheteur — ÉVÉNEMENT REPORTÉ. Billet + QR conservés, valables à la
// nouvelle date. Affiche l'ancienne ET la nouvelle date.
export function eventPostponedEmail(event, { organizerName, previousWhen, newWhen } = {}, site = DEFAULT_SITE) {
  const evName = escapeHtml(event.name || 'ton événement')
  const orga = escapeHtml(organizerName || "L'organisateur")
  const oldW = escapeHtml(previousWhen || '')
  const newW = escapeHtml(newWhen || '')
  const inner = `
    ${h('Événement reporté', '#c8a96e')}
    ${p(`${orga} a <strong style="color:#ffffff;">reporté</strong> « ${evName} » à une nouvelle date.`)}
    ...
    ${p('Bonne nouvelle : <strong style="color:#ffffff;">ton billet reste valable</strong> pour la nouvelle date — rien à faire, garde ton QR.')}
    ${btn(`${site}/evenements/${encodeURIComponent(event.id)}`, 'Voir l\'événement', '#4ee8c8')}
  `
  return { subject: `Reporté : ${event.name || 'ton événement'}`, html: wrap(inner, site) }
}
