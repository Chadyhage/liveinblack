...
async function markFailed(db, ref, { eventId, eventName, sellerUid, amount, reason, code }) {
  await ref.set({
    status: 'failed', failReason: String(reason || 'unknown').slice(0, 300),
    // failCode : cause machine (pour l'auto-guérison #70). 'no_momo_number' /
    // 'country_undetermined' sont ré-armables dès que l'orga corrige ; les autres
    // (event annulé/supprimé, envoi refusé, bloqué) ne le sont pas automatiquement.
    failCode: String(code || 'unknown'),
...
// Ré-arme les enveloppes tombées en échec faute de numéro (« no_momo_number ») ou
// pays indéterminé, DÈS QUE la cause est levée : l'organisateur a ajouté le numéro
// du pays (ou corrigé la région). Elles repassent en 'accumulating' → payées à la
// prochaine passe. Sans ça, l'argent restait bloqué jusqu'à une action admin (#70).
// Appelée par le cron (tous les vendeurs) ET à la volée quand un orga enregistre
// un numéro (sellerUid ciblé, versement quasi immédiat). Ne touche PAS les échecs
// non ré-armables (event annulé/supprimé, envoi refusé, bloqué 48 h).
// Requête mono-champ (sellerUid OU status) → aucun index composite requis.
export async function rearmFailedPayouts(db, sellerUid = null) {
  const q = sellerUid
    ? db.collection('event_payouts').where('sellerUid', '==', String(sellerUid))
    : db.collection('event_payouts').where('status', '==', 'failed')
  const docs = (await q.get()).docs.filter(d => d.data().status === 'failed')
  let rearmed = 0
  for (const d of docs) {
    const ep = d.data()
    ...
    if (!evSnap.exists || evSnap.data().cancelled === true) continue // event supprimé/annulé → pas de ré-arm
    const eventCountry = ep.momoCountry || momoCountryFromRegionName(evSnap.data().region)
    if (!eventCountry) continue // toujours pas de pays → laisse en échec
    const uSnap = await db.collection('users').doc(String(ep.sellerUid)).get()
    const momo = parsePayoutMomoForCountry(uSnap.exists ? uSnap.data() : {}, eventCountry)
    if (!momo) continue // toujours pas de numéro pour ce pays → laisse en échec
    // Ré-arm TRANSACTIONNEL : on re-lit le status au commit et on ne repasse en
    // 'accumulating' QUE s'il est encore 'failed' ré-armable. Sans ça, deux passes
    // concurrentes (ou le ré-arm à la volée + le cron) pourraient écraser un
    // 'paying'/'paid' déjà claim par l'autre → doc incohérent (#72).
    let didRearm = false
    await db.runTransaction(async (tx) => {
      ...
      if (cur.failCode !== 'no_momo_number' && cur.failCode !== 'country_undetermined') return
      tx.set(d.ref, {
        status: 'accumulating', failReason: null, failCode: null,
        momoCountry: eventCountry, claimedAt: null, pendingPayoutId: null,
        updatedAt: FieldValue.serverTimestamp(),
      }, { merge: true })
      didRearm = true
    })
...
// ── Verrou d'exécution du cron payouts (#72) ─────────────────────────────────
// Le claim par enveloppe empêche déjà le double-versement, MAIS deux passes
// concurrentes (Vercel relance un cron en timeout, exécution manuelle +
// planifiée, retry) referaient tout le travail en double et pourraient se courir
// dessus. Ce verrou garantit UNE seule passe à la fois. Il EXPIRE (LOCK_TTL_MS)
// pour ne jamais rester bloqué si une passe crashe sans le libérer.
const PAYOUT_LOCK_ID = 'event_payouts'
const LOCK_TTL_MS = 15 * 60 * 1000 // large : une passe finit bien avant

async function acquirePayoutLock(db, runId, now) {
  const ref = db.collection('cron_locks').doc(PAYOUT_LOCK_ID)
  let acquired = false
  await db.runTransaction(async (tx) => {
    const s = await tx.get(ref)
    const l = s.exists ? s.data() : null
    // Verrou frais tenu par une autre passe → on renonce.
    if (l && l.heldBy && Number(l.expiresAt || 0) > now) return
    tx.set(ref, { heldBy: runId, acquiredAt: now, expiresAt: now + LOCK_TTL_MS, updatedAt: FieldValue.serverTimestamp() }, { merge: true })
    acquired = true
  })
  return acquired
}

async function releasePayoutLock(db, runId) {
  const ref = db.collection('cron_locks').doc(PAYOUT_LOCK_ID)
  // Ne libère QUE si le verrou est toujours le nôtre (une passe qui a expiré et
  // dont le verrou a été repris par une autre ne doit pas le lui arracher).
  await db.runTransaction(async (tx) => {
    const s = await tx.get(ref)
    if (s.exists && s.data().heldBy === runId) {
      tx.set(ref, { heldBy: null, expiresAt: 0, updatedAt: FieldValue.serverTimestamp() }, { merge: true })
    }
  }).catch(() => {})
}
...
export async function processEventPayouts(db, now = Date.now()) {
  const out = { scanned: 0, paid: 0, failed: 0, waiting: 0, skipped: 0, rearmed: 0, locked: false }
  if (!isFedapayConfigured()) { out.skipped = -1; return out } // env absente → rien à faire

  // Verrou d'exécution : une seule passe à la fois (#72).
  const runId = `${now}-${Math.random().toString(36).slice(2, 10)}`
  if (!(await acquirePayoutLock(db, runId, now))) { out.locked = true; return out }

  try {
  // Auto-guérison (#70) : ré-armer les enveloppes en échec « pas de numéro ».
  try { out.rearmed = await rearmFailedPayouts(db) } catch (e) { console.error('[event-payouts] ré-armement échoué:', e.message) }
  ...
  for (const d of snap.docs) {
    ...
    try {
      ...
      if (!eventCountry) {
        await markFailed(db, d.ref, { eventId, eventName: ep.eventName || ev.name, sellerUid, amount, code: 'country_undetermined', reason: `Pays de l'événement indéterminé (région « ${ev.region || '?'} ») — versement Mobile Money impossible sans le pays. À régler à la main / corriger la région.` })
        out.failed++
        continue
      }
      ...
      if (!momo) {
        await markFailed(db, d.ref, { eventId, eventName: ep.eventName || ev.name, sellerUid, amount, code: 'no_momo_number', reason: `Aucun numéro Mobile Money enregistré pour ${countryName} (Profil → Encaissement)` })
        await notifySeller(db, sellerUid, {
          type: 'payout_failed',
    ...
    await releasePayoutLock(db, runId) // toujours libérer, même si une erreur remonte
  }
  return out
}
