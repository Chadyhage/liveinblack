...
export function parsePayoutMomoForCountry(u, momoCountry) {
  ...
  if (momoCountry && map[momoCountry]) {
    const e = cleanMomoEntry(map[momoCountry], momoCountry)
    if (e) return e
  }
  // Repli legacy : ancien numéro unique, uniquement si même pays (ou pays inconnu).
  const legacy = cleanMomoEntry(u?.payoutMomo)
  if (legacy && (!momoCountry || legacy.country === momoCountry)) return legacy
  return null
}
...
export async function processEventPayouts(db, now = Date.now()) {
  ...
  for (const d of snap.docs) {
    ...
    try {
      ...
      const countryName = regionByMomoCountry(eventCountry)?.name || ev.region || 'ce pays'
