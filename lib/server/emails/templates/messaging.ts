...
export function newMessageDigestEmail(senderName: string, preview: string, conversationUrl: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`<span style="display:block;padding:14px 16px;background:${C.surfaceSecondary};border-radius:12px;color:${C.textMuted};font-style:italic;">“${escapeHtml(preview)}”</span>`)}
