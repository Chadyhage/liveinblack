...
export function staffAddedEmail(eventName: string, roleLabel: string, scannerUrl: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`Tu as été ajouté à l'équipe de <strong style="color:${C.text};">${evName}</strong> avec le rôle <strong style="color:${C.text};">${escapeHtml(roleLabel)}</strong>.`)}
...
export function staffRemovedEmail(eventName: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`Tu as été retiré de l'équipe de <strong style="color:${C.text};">${evName}</strong>.`)}
  `
  return {
