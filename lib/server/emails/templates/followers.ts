...
export function organizerNewEventEmail(event: FollowedEventSummary, organizerName: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${note(`Tu reçois cet email parce que tu es abonné à ${name} sur LIVEINBLACK. Tu peux te désabonner ou régler tes alertes depuis <a href="${site}/profile/followed-organizers" style="color:${C.primaryText};">tes organisateurs suivis</a>.`)}
  `
...
export function organizerScheduleChangeEmail(
  ...
  if (kind === 'cancelled') {
    ...
    const inner = `
      ...
      ${note(`Tu reçois cet email parce que tu es abonné à ${name} sur LIVEINBLACK. Règle tes alertes depuis <a href="${site}/profile/followed-organizers" style="color:${C.primaryText};">tes organisateurs suivis</a>.`)}
    `
