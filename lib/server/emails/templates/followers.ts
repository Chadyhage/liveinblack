...
export function organizerNewEventEmail(event: FollowedEventSummary, organizerName: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ${heading(event.name || 'Nouvel événement', 'accent')}
    ${paragraph(`<strong style="color:${C.text};">${name}</strong> vient d'annoncer un nouvel événement sur LIVEINBLACK.`)}
    ...
    ${note(`Tu reçois cet email parce que tu es abonné à ${name} sur LIVEINBLACK. Tu peux te désabonner ou régler tes alertes depuis <a href="${site}/profile/followed-organizers" style="color:${C.primary};text-decoration:none;">tes organisateurs suivis</a>.`)}
  `
...
export function organizerScheduleChangeEmail(
  ...
  if (kind === 'cancelled') {
    ...
    const inner = `
      ...
      ${note(`Tu reçois cet email parce que tu es abonné à ${name} sur LIVEINBLACK. Règle tes alertes depuis <a href="${site}/profile/followed-organizers" style="color:${C.primary};text-decoration:none;">tes organisateurs suivis</a>.`)}
    `
  ...
  const inner = `
    ...
    ${oldW ? paragraph(`<span style="color:${C.textFaint};text-decoration:line-through;">${oldW}</span>`) : ''}
    ${newW ? paragraph(`<strong style="color:${C.primary};">Nouvelle date : ${newW}</strong>`) : ''}
