...
export function eventCancelledRefundEmail(eventName: string, amountLabel: string, delayLabel: string, reason: string | null, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`<strong style="color:${C.text};">${evName}</strong> a été annulé. Tu es automatiquement remboursé de <strong style="color:${C.text};">${amountLabel}</strong>.`)}
    ${reason ? paragraph(`<strong style="color:${C.text};">Motif :</strong> ${escapeHtml(reason)}`) : ''}
...
export function eventPostponedTicketHolderEmail(eventName: string, previousWhen: string, newWhen: string, refundUrl: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`<strong style="color:${C.text};">${evName}</strong> a été reporté à une nouvelle date. Ton billet reste valable.`)}
    ${paragraph(`<span style="color:${C.textFaint};text-decoration:line-through;">${escapeHtml(previousWhen)}</span><br/><strong style="color:${C.primary};">Nouvelle date : ${escapeHtml(newWhen)}</strong>`)}
...
export function refundConfirmedEmail(eventName: string, amountLabel: string, delayLabel: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`Ton remboursement de <strong style="color:${C.text};">${amountLabel}</strong> pour <strong style="color:${C.text};">${evName}</strong> a été traité.`)}
...
export function refundFailedEmail(eventName: string, reason: string | null, supportUrl: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`Le remboursement de ton billet pour <strong style="color:${C.text};">${evName}</strong> n'a pas pu être traité${reason ? ` (${escapeHtml(reason)})` : ''}.`)}
...
export function ticketInvalidatedByResaleEmail(eventName: string, site: string = DEFAULT_SITE): Email {
  ...
  const inner = `
    ...
    ${paragraph(`Ton billet pour <strong style="color:${C.text};">${evName}</strong> a été vendu et n'est plus valable sur ton compte.`)}
  `
  return {
