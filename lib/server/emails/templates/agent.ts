...
export function newApplicationToReviewEmail(candidateName: string, type: 'organisateur' | 'prestataire', backofficeUrl: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`<strong style="color:inherit;">${escapeHtml(candidateName)}</strong> a soumis un dossier de candidature ${type}.`)}
...
export function newReportToReviewEmail(contentTypeLabel: string, backofficeUrl: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`Un signalement a été déposé concernant : <strong style="color:inherit;">${escapeHtml(contentTypeLabel)}</strong>.`)}
...
export function deletionRequestToReviewEmail(userLabel: string, legalDelayLabel: string, backofficeUrl: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`<strong style="color:inherit;">${escapeHtml(userLabel)}</strong> a demandé la suppression de son compte.`)}
...
export function cashSalePendingSettlementEmail(eventName: string, amountLabel: string, daysOverdue: number, backofficeUrl: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`Une vente cash de <strong style="color:inherit;">${amountLabel}</strong> pour <strong style="color:inherit;">${escapeHtml(eventName)}</strong> est en attente de règlement depuis ${daysOverdue} jour(s).`)}
