...
export function applicationReceivedEmail(email: string, site: string = DEFAULT_SITE, type: ApplicationType = 'organisateur'): Email {
  const inner = `
    ...
    ${paragraph(`Ton dossier de candidature ${TYPE_LABEL[type]} a bien été transmis à l'équipe LIVEINBLACK. Tu seras contacté à <strong style="color:${C.text};">${email}</strong> une fois ton compte validé.`)}
...
export function applicationRejectedEmail(type: ApplicationType, reason: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${reason ? paragraph(`<strong style="color:${C.text};">Motif :</strong> ${reason}`) : ''}
  `
...
export function applicationNeedsChangesEmail(type: ApplicationType, requestedChanges: string, site: string = DEFAULT_SITE): Email {
  const inner = `
    ...
    ${paragraph(`<strong style="color:${C.text};">À corriger :</strong> ${requestedChanges}`)}
