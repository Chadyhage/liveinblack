...
//
// Décisions fondateur (2026-06-17):
//   - Frais de service BILLETS = 5% + 0,49 € PAR BILLET, plafonné 2,50 €/billet,
//     payé par l'ACHETEUR, gratuit sur les billets gratuits.
//   - Commission SERVICES prestataires = 10% (activée seulement quand le paiement
//     passe par la plateforme — pas encore branché).
...
// Commission services en centimes (sur le sous-total prestataire).
export function computeServiceFeeCents(subtotalCents) {
  const s = Math.round(Number(subtotalCents) || 0)
  if (s <= 0) return 0
  return Math.round(s * FEES.SERVICE.pct)
}
