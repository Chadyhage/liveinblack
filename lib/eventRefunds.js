...
// ET renverse le crédit vendeur XOF (symétrique au rail EUR). Pas d'appel API
// (FedaPay n'en a pas) : on prépare le travail pour le dashboard.
export async function recordFedapayRefund(db, FieldValue, { eventId, paymentRef, tickets }) {
  const docRef = db.collection('event_refunds').doc(refundDocId(eventId, paymentRef))
  const existing = await docRef.get()
  ...
  const sellerUid = String(txn.sellerUid || '')
  const owedXOF = Math.max(0, amountXOF - (Number(txn.feeAmount) || 0))
  // Renverser le crédit vendeur XOF (audit devise #1) : sinon la recette d'un
  // event FedaPay annulé reste affichée « due » à l'organisateur (seller_balances
  // .amountDueXOF) et pouvait lui être versée manuellement EN PLUS du remboursement
  // des acheteurs = double sortie. On ne renverse QUE si le vendeur a réellement
  // été crédité (fedapay_txns.settled === true) — un paiement capté par une garde
  // webhook AVANT le crédit (event supprimé/annulé pendant le webhook) n'a rien à
  // renverser. Idempotent via ledgerReversedXOF + transaction.
  await db.runTransaction(async (tx) => {
    const snap = await tx.get(docRef)
    const already = snap.exists && snap.data().ledgerReversedXOF === true
    const doReverse = !!FieldValue && sellerUid && owedXOF > 0 && txn.settled === true && !already
    // Lire le solde AVANT d'écrire (transaction) pour CLAMPER à 0 : si l'event a
    // déjà été payé automatiquement (finalizePaid a décrémenté seller_balances)
    // puis annulé, un increment(-owed) aveugle rendrait le solde NÉGATIF (dette
    // fantôme qui rognerait les ventes futures). max(0, dû - owed) reste juste.
    const balRef = doReverse ? db.collection('seller_balances').doc(sellerUid) : null
    const balDue = doReverse
      ? Math.max(0, Number((await tx.get(balRef)).data()?.amountDueXOF || 0) || 0)
      : 0
    if (doReverse) {
      tx.set(balRef, {
        amountDueXOF: Math.max(0, balDue - owedXOF), updatedAt: FieldValue.serverTimestamp(),
      }, { merge: true })
    }
    tx.set(docRef, {
      ...
      ledgerReversedXOF: already || doReverse,
...
export async function processEventRefunds(stripe, db, FieldValue, { eventId, tickets }) {
  ...
  for (const g of fedapayGroups) {
    try {
      const r = await recordFedapayRefund(db, FieldValue, { eventId, paymentRef: g.paymentRef, tickets: g.tickets })
