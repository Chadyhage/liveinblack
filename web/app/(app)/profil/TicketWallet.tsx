...
function EventTicketGroupCard({ group, currentUserId, bucket }: { group: TicketWalletGroupView; currentUserId: string; bucket: GroupBucket }) {
  ...
  return (
    <div style={{ ...cardStyle, overflow: 'hidden' }}>
      <Link
        href={event ? `/evenements/${event.id}` : '#'}
...
function PremiumTicketCard({
  ...
  async function handleShare() {
    if (!event) return
    const result = await shareOrCopy(`${SITE}/evenements/${event.id}`, `Rejoins-moi à ${event.name}`)
        ...
        {inactive ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {event && (
                <Link
                  href={`/commander/${event.id}/${ticket.ticketCode}`}
