...
export default function StudioClient({
  ...
  const publicUrl = typeof window !== 'undefined' ? `${window.location.origin}/organisateurs/${slug}` : `/organisateurs/${slug}`
    ...
    <main style={{ maxWidth: 1180, margin: '0 auto', padding: '30px 20px 100px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '12px 14px', border: '1px solid var(--border)', borderRadius: 12, background: 'var(--surface)', marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        {profile.status === 'public' && (
          <Link href={`/organisateurs/${slug}`} style={{ padding: '8px 14px', borderRadius: 8, background: 'var(--gold)', color: 'var(--obsidian)', fontSize: 11.5, fontWeight: 700, textDecoration: 'none' }}>
            Voir ma page
          </Link>
        )}
      </div>
  ...
  async function connect() {
    ...
    try {
      const res = await fetch('/api/organizers/me/payouts/connect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ returnPath: '/ma-page-organisateur' }),
      })
      const data = await res.json()
