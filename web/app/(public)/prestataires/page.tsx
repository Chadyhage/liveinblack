...
export default async function PublicPrestatairesPage({ searchParams }: { searchParams: Promise<{ q?: string; categorie?: string }> }) {
  ...
  return (
    <div style={{ padding: '28px 22px 60px', maxWidth: 1120, margin: '0 auto', width: '100%' }}>
      ...
      <form action="/prestataires" method="get" style={{ display: 'flex', gap: 8, maxWidth: 420, marginBottom: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
        <CategoryChip label={`Tous (${providers.length})`} href={`/prestataires${search ? `?q=${encodeURIComponent(search)}` : ''}`} active={!category} />
        {PROVIDER_CATEGORIES.filter((c) => counts.get(c.id)).map((c) => (
          <CategoryChip
            key={c.id}
            label={`${c.label} (${counts.get(c.id) || 0})`}
            href={`/prestataires?categorie=${c.id}${search ? `&q=${encodeURIComponent(search)}` : ''}`}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(230px, 1fr))', gap: 16 }}>
          {filtered.map((p) => {
            ...
            return (
              <Link
                key={p.userId}
                href={`/prestataires/${encodeURIComponent(p.userId)}`}
