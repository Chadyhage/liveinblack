      ...
      <div style={{ padding: '0 22px', marginTop: -32, position: 'relative' }}>
          ...
          {upcoming.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
              {upcoming.map((e) => {
                ...
                return (
                  <Link key={e.id} href={`/evenements/${e.id}`} style={{ display: 'block', textDecoration: 'none', color: 'inherit', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        {past.length > 0 && (
          <Section title="Événements passés">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
              {past.map((e) => (
                <Link key={e.id} href={`/evenements/${e.id}`} style={{ display: 'block', textDecoration: 'none', color: 'inherit', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px 12px', opacity: 0.75 }}>
