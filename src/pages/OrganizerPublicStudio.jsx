...
export default function OrganizerPublicStudio() {
  ...
  return <Layout><div className="org-studio">
    ...
    <div className="os-layout">
      ...
      <section className="os-panel os-media-panel"><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}><div><h2 style={{marginBottom:4}}>Galerie photos & vidéos</h2><p style={{margin:0,color:'rgba(255,255,255,.42)',fontSize:12}}>Images 10 Mo max. Vidéos 50 Mo max. Maximum conseillé : 12 médias.</p></div><label className={`os-upload-btn primary${uploading==='gallery'?' busy':''}`}>{uploading==='gallery'?'Envoi…':'+ Ajouter un média'}<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" disabled={!!uploading} onChange={e=>upload('gallery',e.target.files?.[0])}/></label></div>
