        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 20 }}>
            {[
              ...
              { label: 'Organisateurs', path: '/organisateurs', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3 20v-2a6 6 0 0 1 12 0v2"/><path d="M17 8h4m-2-2v4"/></svg> },
              ...(user ? [
                  ...
                  return (
                    <button key={role} onClick={() => !isActive && handleSwitchRole(role)} disabled={isActive || switching}
                      style={{
                        ...
                        background: isActive ? 'rgba(132,68,255,0.12)' : 'rgba(255,255,255,0.04)',
                        border: isActive ? '1px solid rgba(132,68,255,0.28)' : '1px solid rgba(255,255,255,0.08)',
                        transition: 'all 0.2s', opacity: switching ? 0.6 : 1,
                      }}>
                      <RoleIconSquare role={role} />
                      <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 600, color: isActive ? '#c9b0ff' : 'rgba(255,255,255,0.7)', margin: 0 }}>{cfg.label}</p>
                        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: 'rgba(255,255,255,0.4)', margin: 0 }}>{cfg.desc}</p>
                      </div>
                      {isActive && (
          ...
          {user && isAgent && (
            <button onClick={() => go('/agent')} style={{ width: '100%', padding: '14px 16px', borderRadius: 12, background: 'rgba(200,169,110,0.12)', border: '1px solid rgba(200,169,110,0.35)', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', textAlign: 'left', marginBottom: 12 }}>
              <span style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(200,169,110,0.14)', border: '1px solid rgba(200,169,110,0.3)' }}>
