...
export default async function handler(req, res) {
  ...
  const { uid, authOnly = false } = req.body || {}
  ...
  // ── 1. Firebase Auth — libère l'email (l'étape qui manquait) ──
  ...
  try {
    ...
    // user-not-found = déjà supprimé côté Auth (fantôme inverse) — on continue le nettoyage
  }

  if (authOnly) {
    console.log(`[admin-delete-account] ${caller.email} a libéré l'email de ${uid} (${deletedEmail || '?'}) — authOnly`)
    return res.status(200).json({ ok: true, authDeleted, deletedEmail, firestoreDeleted: 0, applicationsDeleted: 0 })
  }

  // ── 2. Firestore — docs par uid ──
  const results = await Promise.allSettled(
    UID_COLLECTIONS.map(col => db.doc(`${col}/${uid}`).delete())
  )
  const firestoreDeleted = results.filter(r => r.status === 'fulfilled').length

  // ── 3. Firestore — dossiers de candidature liés à cet uid ──
  ...
  return res.status(200).json({ ok: true, authDeleted, deletedEmail, firestoreDeleted, applicationsDeleted })
}
