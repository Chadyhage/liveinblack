...
const checks = [
  ...
  },
  {
    ...
    ok: liveActivationGateItems.length >= 5 && Boolean(pkg.scripts?.['audit:vercel-live-gates']) && readText('app/api/agent/vercel/ops-config/route.ts').includes('remainingGates') && readText('app/api/agent/vercel/ops-config/route.ts').includes('getLiveEvidenceMatrixSummary') && readText('app/api/agent/vercel/ops-config/route.ts').includes('getNextProofCaptureSummary') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Portes live restantes') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Matrice des preuves live') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Prochaine preuve à capturer') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('fermée(s)'),
    evidence: `${liveActivationGateItems.length} porte(s) live reliees au 100%, filtrees par decisions et visibles agent avec matrice de preuves live et prochaine preuve a capturer`,
  },
  {
    label: 'Ordre activation 100% Vercel Pro',
    ok: activationOrderSteps.length >= liveActivationGateItems.length && Boolean(pkg.scripts?.['audit:vercel-activation-order']) && readText('app/api/agent/vercel/ops-config/route.ts').includes('remainingSteps') && readText('app/api/agent/vercel/ops-config/route.ts').includes('actionBlockers') && readText('app/api/agent/vercel/ops-config/route.ts').includes('getRiskCostSummary') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Ordre recommandé') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Blocages vers 100%') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Risque et coût restants') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('séquence restante'),
    evidence: `${activationOrderSteps.length} etape(s) ordonnees, filtrees et resumees par type de blocage, risque et cout`,
  },
  {
    label: 'Preuves completion 100% Vercel Pro',
    ok: completionRequirements.length >= 5 && Boolean(pkg.scripts?.['audit:vercel-completion']) && Boolean(pkg.scripts?.['ops:vercel:evidence:record']) && readText('app/api/agent/vercel/ops-config/route.ts').includes('evidenceRecordCommand') && readText('app/api/agent/vercel/ops-config/route.ts').includes('getCompletionVerdictSummary') && readText('app/api/agent/vercel/ops-config/route.ts').includes('getProUtilizationModeSummary') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Preuves du 100%') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Verdict 100% Vercel Pro') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Mode d’exploitation Vercel Pro') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Commande preuve') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Copier'),
    evidence: `${completionRequirements.length} exigence(s) de preuve avant de declarer 100%, visibles agent avec verdict 100%, mode exploitation et commande de consignation copiable`,
  },
  {
    label: 'Suite audit 100% Vercel Pro',
    ok: auditSuiteCommands.length >= 10 && Boolean(pkg.scripts?.['audit:vercel-pro-suite']) && Boolean(pkg.scripts?.['ops:vercel:next-action']) && auditSuiteCommands.some((item) => item.scope === 'live-read') && auditSuiteCommands.some((item) => item.expectedBeforeLiveComplete === 'fail-until-live-proof') && readText('app/api/agent/vercel/ops-config/route.ts').includes('auditSuite') && readText('app/api/agent/vercel/ops-config/route.ts').includes('dashboardTargetForGate') && readText('app/api/agent/vercel/ops-config/route.ts').includes('safetyForAutomationLevel') && readText('app/api/agent/vercel/ops-config/route.ts').includes('buildPreflightChecklist') && readText('app/api/agent/vercel/ops-config/route.ts').includes('preflightChecklist') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Suite d’audit 100%') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Prochaine action recommandée') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('CommandLine') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Sécurité') && readText('app/components/features/agent/AgentVercelOpsClient.tsx').includes('Checklist avant action'),
    evidence: `${auditSuiteCommands.length} audit(s) orchestral(s), live optionnel, completion stricte, prochaine action, checklist preflight, niveau securite, dashboard contextuel et commandes copiables visibles agent`,
  },
]
