...
// Le repo n'est pas en "type": "module" — un top-level await échoue à la
// transpilation esbuild ("Top-level await is currently not supported with
// the cjs output format"), y compris via `npm run cloudinary:setup`. Tout
// dans une fonction async classique, comme scripts/seed-dev.ts.
async function main() {
  ...
  async function upsertPreset(name: string, options: Record<string, unknown>): Promise<void> {
    try {
      ...
      // Le SDK Cloudinary imbrique le code HTTP dans error.error.http_code
      // (pas error.http_code directement) — la vérification précédente ne
      // matchait donc jamais 404 et relançait systématiquement l'erreur au
      // lieu de créer le preset manquant.
      const httpCode = (error as { http_code?: number; error?: { http_code?: number } }).error?.http_code ?? (error as { http_code?: number }).http_code
